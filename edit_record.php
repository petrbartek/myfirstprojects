...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$errors[] = 'You forgot to enter grade.';
	} else {
	$g = mysqli_real_escape_string($dbcon, trim($_POST['grade']));
	}
	if (empty($errors)) { // If everything's OK
		...
		// Make the update query:
			$q = "UPDATE users SET fname='$fn', lname='$ln', email='$e', grade='$g' WHERE user_id=$id LIMIT 1";
			$result = @mysqli_query ($dbcon, $q);
...
$q = "SELECT fname, lname, email, grade FROM users WHERE user_id=$id";		
$result = @mysqli_query ($dbcon, $q);
...
<br><p><label class="label" for="grade">grade:</label><input class="fl-left" type="text" name="grade" size="30" maxlength="40" value="' . $row[3] . '"></p>
